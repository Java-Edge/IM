...
import com.javaedge.im.codec.pack.message.MessageReciveServerAckPack;
...
import com.javaedge.im.service.message.model.req.SendMessageReq;
import com.javaedge.im.service.message.model.resp.SendMessageResp;
...
public class P2PMessageService {
    ...
    public void process(MessageContent messageContent) {
        ...
        // 这个用户是否被禁言 是否被禁用
        // 发送方和接收方是否是好友
//        ResponseVO responseVO = imServerPreCheck(fromId, toId, appId);
//        if(responseVO.isOk()){
        threadPoolExecutor.execute(() -> {
...
//        }
    ...
    private List<ClientInfo> dispatchMessage(MessageContent messageContent) {
        ...
        return msgProducer.send2User(messageContent.getToId(), MessageCommand.MSG_P2P,
    ...
    private void ack(MessageContent messageContent, ResponseVO responseVO) {
        ...
        msgProducer.send2User(messageContent.getFromId(), MessageCommand.MSG_ACK,
    ...
    public void receiverAck(MessageContent messageContent) {
        ...
        msgProducer.send2User(messageContent.getFromId(), MessageCommand.MSG_RECEIVE_ACK,
                pack, new ClientInfo(messageContent.getAppId(), messageContent.getClientType()
                        , messageContent.getImei()));
    }

    private void syncToSender(MessageContent messageContent, ClientInfo clientInfo) {
        msgProducer.send2UserExceptClient(messageContent.getFromId(),
                MessageCommand.MSG_P2P, messageContent, messageContent);
    }


    public ResponseVO imServerPreCheck(String fromId, String toId, Integer appId) {
